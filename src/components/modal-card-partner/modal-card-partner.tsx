...
export const ModalCardPartner = ({item, onClose}: ModalCardPartnerProps) => {
    ...
    <div className='modal-card-partner' data-test-id='partner-modal'>
        <div className='modal-card-partner__content'>
            <Button type='text' style={{position: 'absolute', top: '16px', right: '10px'}} onClick={handleClick}>
        ...
        {item.imageSrc
            ...
            <div className='fullName'>
                <div className='name'>
                    {item.name ? item.name.split(' ')[0] : 'Пользователь'}
                </div>
                <div className='surname'>{item.name && item.name.split(' ')[1]}</div>
            </div>
        </div>
...
<div className='wrapper'>
<div className='message'>тренировка одобрена<CheckCircleFilled style={{color: 'var(--character-light-success)', fontSize: '14px', paddingLeft: '9px'}}/></div>
<Button  size='large' className='btn__cancel-training' onClick={handleButtonCancel}>Отменить тренировку</Button>
