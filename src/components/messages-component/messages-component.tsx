...
export const MessagesComponent = () => {
    ...
    <div className='messages-component'>
        ...
        {(showAll ? inviteList : [inviteList[0]]).map((item) => <CardMessage item={item}  key={item._id}/>)}
        </div>

        {inviteList.length > 1 && showAll?
        <Button type='link'
            onClick={handleHiddenAllClick}
            className='btn_show-message'
            >
        <UpOutlined style={{fontSize: '8px', paddingRight: '2px'}}/>
        Скрыть все сообщения</Button>:
        <Button type='link'
