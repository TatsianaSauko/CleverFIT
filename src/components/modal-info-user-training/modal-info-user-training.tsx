...
export const ModalInfoUserTraining = ({ closeClick, position, item}: ModalInfoUserTrainingProps) => {
    const period = periodOptions.find(option => option.value === item.training.parameters?.period);
    ...
    return (
        ...
        >
            ...
            <div className='info-user-training__content'>
                ...
                <div className='value__date'>{moment(item.createdAt).format('DD.MM.YYYY')}</div>
                </div>
<div className='wrapper'>
<div className='type__wrapper'>

{item.training.exercises.map((exercise) => <div className='type'  key={exercise.name}>{exercise.name}</div>)}
...
{item.training.exercises.map((value) =>
    <div className='value' key={value._id}>
        {value.replays} x {value.weight ? `(${value.weight} кг)` : value.approaches}
    </div>
)}
